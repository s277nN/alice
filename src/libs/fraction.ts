import { Fraction as SDKFraction } from '@pancakeswap/sdk'
import { BigNumber, BigNumberish } from '@ethersproject/bignumber'
import { One, Zero } from '@ethersproject/constants'
import { formatBalance, parseBalance } from '@/libs/format'
...
export class Fraction {
  ...
  // Parse BigNumber to Fraction via [/ denominator]
  static from(numerator: BigNumberish, denominator?: BigNumberish): Fraction {
    return new Fraction(BigNumber.from(numerator), BigNumber.from(denominator || this.BASE))
  }

  // Parse string to Fraction via [* 1e18]
  ...
  constructor(numerator: BigNumber, denominator?: BigNumber) {
    ...
    this.denominator = denominator || BigNumber.from(10).pow(18)
  }
